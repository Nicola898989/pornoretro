...
const RetroCard: React.FC<RetroCardProps> = ({
  ...
  return <Card className={`bg-gradient-to-br from-pornoretro-black to-pornoretro-black/80 text-white border-2 border-pornoretro-darkorange ${inGroup ? 'opacity-80' : ''}`} draggable={!isEditing && !inGroup} onDragStart={handleDragStart} onDrop={handleDrop} onDragOver={e => e.preventDefault()}>
      <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2">
        <div className="space-y-1 flex-1">
          <p className="text-xs text-muted-foreground">di {author}</p>
          {isEditing ? <div className="flex flex-col space-y-2">
              <Textarea ref={textareaRef} value={newContent} onChange={e => setNewContent(e.target.value)} className="bg-secondary/20 border-secondary/30 text-sm" />
            ...
            </div> : <p className="text-sm leading-relaxed">{content}</p>}
        </div>
        
        <div className="flex items-center gap-1 ml-2">
          {onChangeCategory && <CategorySelector currentType={type} onChangeCategory={handleCategoryChange} disabled={isEditing} />}
          {!isEditing && currentUser === author && !inGroup && onEdit && <Button variant="ghost" size="icon" onClick={handleEdit}>
              <Edit className="h-4 w-4 text-muted-foreground hover:text-pornoretro-orange" />
            </Button>}
          {!isEditing && onDelete && <Button variant="ghost" size="icon" onClick={handleDelete}>
              <Trash2 className="h-4 w-4 text-muted-foreground hover:text-red-500" />
            </Button>}
          {inGroup && onRemoveFromGroup && <Button variant="ghost" size="icon" onClick={handleRemoveFromGroup}>
      ...
      <CardFooter className="text-xs text-muted-foreground px-4 py-2">
        {comments.length > 0 && <div className="mt-2">
            {comments.map(comment => <div key={comment.id} className="mb-1">
